...
function App() {
  ...
  createEffect(() => {
    if (route.data.type === "home") {
      renderer.setTerminalTitle("LEF CODE")
      return
    }
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "docs.open",
      onSelect: () => {
        open("https://github.com/orestislef/lefcode").catch(() => {})
        dialog.clear()
      },
      category: "System",
    },
  ...
  event.on(Installation.Event.Updated.type, (evt) => {
    toast.show({
      ...
      message: `LEF CODE updated to v${evt.properties.version}`,
      duration: 5000,
    })
  })
  ...
  event.on(Installation.Event.UpdateAvailable.type, (evt) => {
    toast.show({
      ...
      message: `LEF CODE v${evt.properties.version} is available. Run 'lefcode upgrade' to update manually.`,
      duration: 10000,
    })
  })
  ...
  return (
    <box
      ...
      onMouseUp={async () => {
        if (Flag.LEFCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) {
...
function ErrorComponent(props: { error: Error; reset: () => void; onExit: () => Promise<void> }) {
  ...
  const issueURL = new URL("https://github.com/orestislef/lefcode/issues/new")
  ...
  issueURL.searchParams.set("lefcode-version", Installation.VERSION)
