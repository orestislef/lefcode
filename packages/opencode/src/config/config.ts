...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.LEFCODE_CONFIG) {
      result = mergeConfigWithPlugins(result, await loadFile(Flag.LEFCODE_CONFIG))
      log.debug("loaded custom config", { path: Flag.LEFCODE_CONFIG })
    }

    for (const file of ["lefcode.jsonc", "lefcode.json"]) {
    ...
    if (Flag.LEFCODE_CONFIG_CONTENT) {
      result = mergeConfigWithPlugins(result, JSON.parse(Flag.LEFCODE_CONFIG_CONTENT))
      log.debug("loaded custom config from LEFCODE_CONFIG_CONTENT")
    }

    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        process.env[value.key] = value.token
        const wellknown = (await fetch(`${key}/.well-known/lefcode`).then((x) => x.json())) as any
    ...
    if (Flag.LEFCODE_CONFIG_DIR) {
      directories.push(Flag.LEFCODE_CONFIG_DIR)
      log.debug("loading config from LEFCODE_CONFIG_DIR", { path: Flag.LEFCODE_CONFIG_DIR })
    }
    ...
    for (const dir of directories) {
      ...
      if (dir.endsWith(".lefcode") || dir === Flag.LEFCODE_CONFIG_DIR) {
        for (const file of ["lefcode.jsonc", "lefcode.json"]) {
    ...
    if (Flag.LEFCODE_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LEFCODE_PERMISSION))
    }
  ...
  async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@lefcode/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const name = (() => {
        const patterns = ["/.lefcode/command/", "/command/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const agentFolderPath = item.includes("/.lefcode/agent/")
        ? item.split("/.lefcode/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
  ...
  // Default LEF CODE provider configuration
  const DEFAULT_CONFIG: Info = {
    provider: {
      "lef-local": {
        name: "LEF Local",
        api: "http://192.168.24.211:1234/v1",
        ...
        options: {
          apiKey: "lef",
          baseURL: "http://192.168.24.211:1234/v1",
        },
      },
    },
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "lefcode.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "lefcode.jsonc"))),
    )
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        // Skip writing schema for LEF CODE
      }
